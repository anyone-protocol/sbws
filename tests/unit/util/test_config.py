...
def test_validate_fingerprint():
    ...
    goods = ["A" * fp_len, "".join(list("0123456789ABCDEF" * 3)[0:fp_len])]
    for fp in bads:
        d = {"": fp}
        valid, reason = con._validate_fingerprint(d, "")
        assert (
            not valid
        ), "Fingerprint {} should not have passed " "validation".format(fp)
    for fp in goods:
        d = {"": fp}
        valid, reason = con._validate_fingerprint(d, "")
        assert valid, (
            "Fingerprint {} should have passed, but didn't "
...
def test_validate_int_simple():
    ...
    goods = [
        ...
        PseudoSection("", "1"),
        PseudoSection("", "-1"),
        PseudoSection("", "100000000"),
        PseudoSection("", "-1000000000"),
        PseudoSection("", "+0"),
        PseudoSection("", "-0"),
    ]
    for sec in bads:
        valid, reason = con._validate_int(sec, sec.key)
        assert not valid, "{} should not have been a valid " "int".format(
            sec.value
        )
    for sec in goods:
        valid, reason = con._validate_int(sec, sec.key)
        assert valid, "{} should have been a valid int, but " "got: {}".format(
            sec.value, reason
        )
...
def test_validate_float_simple():
    ...
    for sec in bads:
        ...
        assert not valid, "{} should not have been a valid " "float".format(
            sec.value
        )
    for sec in goods:
        valid, reason = con._validate_float(sec, sec.key)
        assert (
            valid
        ), "{} should have been a valid float, but " "got: {}".format(
            sec.value, reason
        )
...
def test_validate_bool():
    ...
    for val in goods:
        ...
        assert (
            valid
        ), "{} should have been a valid bool, but " "got: {}".format(
            val, reason
        )
    for val in bads:
        conf = ConfigParser()
        ...
        conf["sec"]["key"] = val
        valid, reason = con._validate_boolean(conf["sec"], "key")
        assert not valid, "{} should not have been a valid " "bool".format(val)
...
def test_validate_url():
    ...
    for val in goods:
        ...
        assert valid, "{} should have been a valid URL, but " "got: {}".format(
            val, reason
        )
    for val in bads:
        d = {"": val}
...
def test_country(conf):
    ...
    assert (
        errors[0] == "destinations.foo/country (None): "
