...
destinations_countries_l = KEYVALUE_SEP_V1.join(
...
latest_bandwidth_l = KEYVALUE_SEP_V1.join(
    ["latest_bandwidth", latest_bandwidth]
)
attempts = "1"
attempts_l = KEYVALUE_SEP_V1.join(
    ["recent_measurement_attempt_count", attempts]
)
failure = "0"
failure_l = KEYVALUE_SEP_V1.join(["recent_measurement_failure_count", failure])
header_ls = [
    timestamp_l,
    version_l,
...
earliest_bandwidth_l = KEYVALUE_SEP_V1.join(
    ["earliest_bandwidth", earliest_bandwidth]
)
generator_started = "2018-04-16T14:09:05"
generator_started_l = KEYVALUE_SEP_V1.join(
...
def test_update_progress(datadir, tmpdir):
    ...
    bwfile.update_progress(
        len(bw_lines_raw), header, number_consensus_relays, state
    )
