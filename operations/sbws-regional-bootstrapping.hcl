...
job "sbws-regional-bootstrapping" {
  ...
  constraint {
    ...
    operator  = "regexp"
    value     = "^(any1-live-jnb-net-auth-1|any1-live-sin-net-auth-1|any1-live-sea-net-auth-1|any1-live-bgr-net-auth-1)$"
  }
  group "sbws-regional-bootstrapping-group" {
    count = 4
    volume "sbws-live" {
      type      = "host"
      read_only = false
    ...
    task "sbws-fileserver-task" {
      ...
      service {
        name = "sbws-regional-bootstrapping"
        port = "http_file_port"
        tags = ["logging"]
        check {
