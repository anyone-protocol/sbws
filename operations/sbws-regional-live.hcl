...
job "sbws-regional-live" {
  ...
  constraint {
    ...
    operator  = "regexp"
    value     = "^(any1-live-chi-net-auth-1|any1-live-lim-net-auth-1|any1-live-jnb-net-auth-1|any1-live-sin-net-auth-1|any1-live-sea-net-auth-1|any1-live-bgr-net-auth-1)$"
  }

  group "sbws-regional-live-group" {
    count = 6
    ...
    task "sbws-relay-task" {
      ...
      template {
        ...
        {{- $loc := "" -}}
        {{- if eq (env "attr.unique.hostname") "any1-live-jnb-net-auth-1" -}}{{- $loc = "Jnb" -}}
        {{- else if eq (env "attr.unique.hostname") "any1-live-sin-net-auth-1" -}}{{- $loc = "Sin" -}}
        {{- else if eq (env "attr.unique.hostname") "any1-live-bgr-net-auth-1" -}}{{- $loc = "Bgr" -}}
        {{- else if eq (env "attr.unique.hostname") "any1-live-sea-net-auth-1" -}}{{- $loc = "Sea" -}}
        {{- else if eq (env "attr.unique.hostname") "any1-live-fra-net-auth-1" -}}{{- $loc = "Fra" -}}
        {{- else if eq (env "attr.unique.hostname") "any1-live-chi-net-auth-1" -}}{{- $loc = "Chi" -}}
        {{- else if eq (env "attr.unique.hostname") "any1-live-lim-net-auth-1" -}}{{- $loc = "Lim" -}}
        {{- end -}}
        Nickname Anyone{{ $loc }}SBWS
    ...
    task "sbws-scanner-task" {
      ...
      template {
        ...
        data        = <<-EOH
        {{- $country := "AA" -}}
        {{- if eq (env "attr.unique.hostname") "any1-live-jnb-net-auth-1" -}}{{- $country = "ZA" -}}
        {{- else if eq (env "attr.unique.hostname") "any1-live-chi-net-auth-1" -}}{{- $country = "US" -}}
        {{- else if eq (env "attr.unique.hostname") "any1-live-lim-net-auth-1" -}}{{- $country = "DE" -}}
        {{- else if eq (env "attr.unique.hostname") "any1-live-sin-net-auth-1" -}}{{- $country = "SG" -}}
        {{- else if eq (env "attr.unique.hostname") "any1-live-bgr-net-auth-1" -}}{{- $country = "BG" -}}
        {{- else if eq (env "attr.unique.hostname") "any1-live-sea-net-auth-1" -}}{{- $country = "US" -}}
        {{- else if eq (env "attr.unique.hostname") "any1-live-fra-net-auth-1" -}}{{- $country = "FR" -}}
        {{- end -}}
    ...
    task "sbws-fileserver-task" {
      ...
      service {
        name = "sbws-bandwidth-${attr.unique.hostname}"
        port = "http_file_port"
        tags = ["logging", "sbws-bandwidth"]
